...
classdef FileContentsReport < pm.io.FileContents
    ...
    properties(Access = public)
        %>
        %>  \param[in]  stats       :   The scalar MATLAB ``struct`` containing the set of
        %>                              computed properties extracted from the report file.
        %>
        stats = struct();
        %>
        %>  \param[in]  contents    :   The scalar MATLAB string containing the entire
        %>                              contents of the report file with all Carriage Return
        %>                              characters removed (relevant only to Windows OS).
        %>
        contents = [];
        %>
        %>  \param[in]  lineList    :   The vector of MATLAB strings containing the set of
        %>                              all lines in the report file with all Carriage Return
        %>                              and New Line characters removed.
        %>
        lineList = [];
        % %
        % %   setup
    ...
    methods(Access = public)
        ...
        %>  \brief
        %>  Return a scalar object of class ``pm.sampling.FileContentsReport``.<br>
