...
%>      chainList = pm.sampling.readChain(sampler, pattern, sep);
%>
%>  \endcode
%>
%>  \note
...
function chainList = readChain(sampler, pattern, sep)
    ...
    for ifile = numel(pathList) : -1 : 1
        ...
        if  isequal(class(sampler), "pm.sampling.Sampler")
            chainList{ifile} = pm.sampling.FileContentsChain(pathList(ifile), sampler.silent, sep);
        elseif isequal(class(sampler), "pm.sampling.Paradram")
            chainList{ifile} = pm.sampling.FileContentsChainDRAM(pathList(ifile), sampler.silent, sep);
        else
