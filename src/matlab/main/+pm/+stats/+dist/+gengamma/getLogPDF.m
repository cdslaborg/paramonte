...
%>  \code{.m}
%>
%>      logPDF = pm.stats.dist.gengamma.getLogPDF(x, kappa, invOmega, invSigma)
%>
%>  \endcode
%>
%>  \example{getLogPDF}
%>  \include{lineno} example/stats/dist/gengamma/getLogPDF/main.m
%>  \output{getLogPDF}
%>  \include{lineno} example/stats/dist/gengamma/getLogPDF/main.out.m
%>  \vis{getLogPDF}
%>  \image html example/stats/dist/gengamma/getLogPDF/gengamma.getLogPDF.line.png width=700
%>
%>  \final{getLogPDF}
%>
%>  \author
...
function logPDF = getLogPDF(x, kappa, invOmega, invSigma)
    ...
    xscaled = x .* invSigma;
    logPDFNF = log(invOmega) + log(invSigma) - gammaln(kappa);
    logPDF = logPDFNF + log(xscaled) .* (kappa .* invOmega - 1) - xscaled.^invOmega;
end