    ...
    if  any(contains(pm.lib.mpi.vendor(mpiVendorList), "Intel")) && ~ismac
        ...
        if  itis
            try
                ...
                binpath = pm.lib.path.auxil();
                webfile = fullfile(binpath, ".deps." + pm.os.namel() + ".mpi.intel");
                binlink = strrep(strrep(fileread(webfile), char(13), ''), char(10), '');

                binpath = websave(binpath, binlink);
