...
function idx = cnClosest(dtE,ord,rgb,inp,typ)
    ...
    assertion = ismatrix(inp) && size(inp, 2) == 3 && isreal(inp) && all(0 <= inp(:) & inp(:) <= 1);
    assert(assertion, 'The %s input argument can be a map of RGB values (size Nx3).', ord{2});
...
function LCHs = cnCIE94(mat,one)
    ...
    LCHs = ([(mat(:,1)-one(:,1)), (Ca1-Ca2), dHa] ./ bsxfun(@times, kLCH, (1 + bsxfun(@times, Ca1, K012)))).^2;
end
...
function LCHsR = cnCIE2k(mat,one)
    ...
    dLCH = [(one(:,1)-mat(:,1)), (Cp2-Cp1), 2*sqrt(Cpp).*sind(dhp/2)] ./ bsxfun(@times, kLCH, 1 + SLCH);
