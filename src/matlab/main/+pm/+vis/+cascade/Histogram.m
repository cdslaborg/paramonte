...
classdef Histogram < pm.vis.cascade.Cascade
    methods(Access = public)
        %>  \brief
        %>  Construct and return an object of class [pm.vis.cascade.Histogram](@ref Histogram).<br>
        %>
        ...
        %><br>
        %>  \return
        %>  ``self``                :   The output scalar object of class [pm.vis.cascade.Histogram](@ref Histogram).<br>
        %>
        %>  \note
        ...
        %>  \code{.m}
        %>
        %>      p = pm.vis.cascade.Histogram(dfref);
        %>      p = pm.vis.cascade.Histogram(dfref, varargin);
        %>
        %>  \endcode
        %>
        %>  \final{Histogram}
        ...
        function self = Histogram(dfref, varargin)
            ...
            self = self@pm.vis.cascade.Cascade(pm.vis.plot.Histogram(dfref), varargin{:});
        end
    end
end