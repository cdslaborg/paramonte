...
%>  instances of 2-dimensional LineScatter [Cascade visualizations](@ref Cascade)
...
classdef LineScatter < pm.vis.cascade.Cascade
    methods(Access = public)
        %>  \brief
        %>  Construct and return an object of class [pm.vis.cascade.LineScatter](@ref LineScatter).<br>
        %>
        ...
        %>
        %>  \return
        %>  ``self``                :   The output scalar object of class [pm.vis.cascade.LineScatter](@ref LineScatter).<br>
        %>
        %>  \note
        ...
        %>  \code{.m}
        %>
        %>      p = pm.vis.cascade.LineScatter(dfref);
        %>      p = pm.vis.cascade.LineScatter(dfref, varargin);
        %>
        %>  \endcode
        %>
        %>  \final{LineScatter}
        ...
        function self = LineScatter(dfref, varargin)
            ...
            self = self@pm.vis.cascade.Cascade(pm.vis.plot.LineScatter(dfref), varargin{:});
        end
    end
end