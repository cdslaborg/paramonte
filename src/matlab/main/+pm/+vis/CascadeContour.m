%>  \brief
%>  This is the CascadeContour class for generating
...
classdef CascadeContour < pm.vis.Cascade
    ...
    methods(Access = public)
        ...
        %>
        %>  \return
        %>  ``self``                :   The output scalar object of class [pm.vis.CascadeContour](@ref CascadeContour).<br>
        %>
        %>  \interface{CascadeContour}
        %>  \code{.m}
        %>
        %>      p = pm.vis.CascadeContour(dfref);
        %>      p = pm.vis.CascadeContour(dfref, varargin);
        %>
        %>  \endcode
        %>
        %>  \note
        ...
        %>
        %>  \example{CascadeContour}
        %>  \include{lineno} example/vis/CascadeContour/main.m
        %>  \vis{CascadeContour}
        ...
        %>  \FatemehBagheri, May 20 2024, 1:25 PM, NASA Goddard Space Flight Center (GSFC), Washington, D.C.<br>
        %>  \AmirShahmoradi, May 16 2016, 9:03 AM, Oden Institute for Computational Engineering and Sciences (ICES), UT Austin<br>
        function self = CascadeContour(dfref, varargin)
            if nargin < 1
                dfref = [];
            end
            self = self@pm.vis.Cascade(pm.vis.PlotContour(dfref), varargin{:});
        end
