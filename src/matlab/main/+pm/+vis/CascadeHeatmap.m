%>  \brief
%>  This is the CascadeHeatmap class for generating
%>  instances of 2-dimensional Heatmap [Cascade visualizations](@ref Cascade)
...
classdef CascadeHeatmap < pm.vis.Cascade

    methods(Access = public)

        %>  \brief
        %>  Construct and return an object of class [pm.vis.CascadeHeatmap](@ref CascadeHeatmap).<br>
        %>
        ...
        %>
        %>  \return
        %>  ``self``                :   The output scalar object of class [pm.vis.CascadeHeatmap](@ref CascadeHeatmap).<br>
        %>
        %>  \interface{CascadeHeatmap}
        %>  \code{.m}
        %>
        %>      p = pm.vis.CascadeHeatmap(dfref);
        %>      p = pm.vis.CascadeHeatmap(dfref, varargin);
        %>
        %>  \endcode
        %>
        %>  \note
        ...
        %>
        %>  \example{CascadeHeatmap}
        %>  \include{lineno} example/vis/CascadeHeatmap/main.m
        %>  \vis{CascadeHeatmap}
        ...
        %>  \FatemehBagheri, May 20 2024, 1:25 PM, NASA Goddard Space Flight Center (GSFC), Washington, D.C.<br>
        %>  \AmirShahmoradi, May 16 2016, 9:03 AM, Oden Institute for Computational Engineering and Sciences (ICES), UT Austin<br>
        function self = CascadeHeatmap(dfref, varargin)
            if nargin < 1
                dfref = [];
            end
            self = self@pm.vis.Cascade(pm.vis.PlotHeatmap(dfref), varargin{:});
        end
