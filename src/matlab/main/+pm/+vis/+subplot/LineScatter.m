...
%>  [pm.vis.subplot.LineScatter](@ref LineScatter::LineScatter) for example usage.<br>
%>
%>  \see
...
classdef LineScatter < pm.vis.subplot.Subplot
    methods(Access = public)
        %>  \brief
        %>  Construct and return an object of class [pm.vis.subplot.LineScatter](@ref LineScatter).<br>
        %>
        %>  \details
        %>  This is the constructor of the class [pm.vis.subplot.LineScatter](@ref LineScatter).<br>
        %>
        ...
        %>
        %>  \return
        %>  An object of [pm.vis.subplot.LineScatter](@ref LineScatter) class.<br>
        %>
        %>  \note
        ...
        %>
        %>  \example{LineScatter}
        %>  \include{lineno} example/vis/subplot/LineScatter/main.m
        %>  \vis{Subplot}
        %>  <br>\image html example/vis/subplot/LineScatter/Subplot.LineScatter.1.png width=700
        %>
        %>  \final{LineScatter}
        %>
        %>  \author
