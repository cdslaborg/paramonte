...
%>  [pm.vis.subplot.Histfit](@ref Histfit::Histfit) for example usage.<br>
%>
%>  \see
...
classdef Histfit < pm.vis.subplot.Subplot
    methods(Access = public)
        %>  \brief
        %>  Construct and return an object of class [pm.vis.subplot.Histfit](@ref Histfit).<br>
        %>
        %>  \details
        %>  This is the constructor of the class [pm.vis.subplot.Histfit](@ref Histfit).<br>
        %>
        ...
        %>
        %>  \return
        %>  An object of [pm.vis.subplot.Histfit](@ref Histfit) class.<br>
        %>
        %>  \note
        ...
        %>
        %>  \example{Histfit}
        %>  \include{lineno} example/vis/subplot/Histfit/main.m
        %>  \vis{Subplot}
        %>  <br>\image html example/vis/subplot/Histfit/Subplot.Histfit.1.png width=700
        %>
        %>  \final{Histfit}
        %>
        %>  \author
