...
%>  [pm.vis.subplot.Line](@ref Line::Line) for example usage.<br>
%>
%>  \see
...
classdef Line < pm.vis.subplot.Subplot
    methods(Access = public)
        %>  \brief
        %>  Construct and return an object of class [pm.vis.subplot.Line](@ref Line).<br>
        %>
        %>  \details
        %>  This is the constructor of the class [pm.vis.subplot.Line](@ref Line).<br>
        %>
        ...
        %>
        %>  \return
        %>  An object of [pm.vis.subplot.Line](@ref Line) class.<br>
        %>
        %>  \note
        ...
        %>
        %>  \example{Line}
        %>  \include{lineno} example/vis/subplot/Line/main.m
        %>  \vis{Subplot}
        %>  <br>\image html example/vis/subplot/Line/Subplot.Line.1.png width=700
        %>
        %>  \final{Line}
        %>
        %>  \author
