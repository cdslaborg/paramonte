...
classdef PlotLineScatter < pm.vis.Plot
    ...
    methods(Access = public)
        ...
        %>  \code{.m}
        %>
        %>      p = pm.vis.PlotLineScatter(dfref);
        ...
        %>
        %>  \author
        %>  \JoshuaOsborne, May 21 2024, 10:10 AM, University of Texas at Arlington<br>
        %>  \FatemehBagheri, May 20 2024, 1:25 PM, NASA Goddard Space Flight Center (GSFC), Washington, D.C.<br>
        %>  \AmirShahmoradi, May 16 2016, 9:03 AM, Oden Institute for Computational Engineering and Sciences (ICES), UT Austin<br>
        function self = PlotLineScatter(dfref, varargin)
            if nargin < 1
                dfref = [];
            end
            self = self@pm.vis.Plot(pm.vis.SubplotLineScatter(dfref), varargin{:});
        end
