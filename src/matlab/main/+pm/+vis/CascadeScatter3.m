%>  \brief
%>  This is the CascadeScatter3 class for generating
%>  instances of 3-dimensional Scatter3 [Cascade visualizations](@ref Cascade)
...
classdef CascadeScatter3 < pm.vis.Cascade

    methods(Access = public)

        %>  \brief
        %>  Construct and return an object of class [pm.vis.CascadeScatter3](@ref CascadeScatter3).<br>
        %>
        ...
        %>
        %>  \return
        %>  ``self``                :   The output scalar object of class [pm.vis.CascadeScatter3](@ref CascadeScatter3).<br>
        %>
        %>  \interface{CascadeScatter3}
        %>  \code{.m}
        %>
        %>      s = pm.vis.CascadeScatter3(dfref);
        %>      s = pm.vis.CascadeScatter3(dfref, varargin);
        %>
        %>  \endcode
        %>
        %>  \note
        ...
        %>  \FatemehBagheri, May 20 2024, 1:25 PM, NASA Goddard Space Flight Center (GSFC), Washington, D.C.<br>
        %>  \AmirShahmoradi, May 16 2016, 9:03 AM, Oden Institute for Computational Engineering and Sciences (ICES), UT Austin<br>
        function self = CascadeScatter3(dfref, varargin)
            if nargin < 1
                dfref = [];
            end
            self = self@pm.vis.Cascade(pm.vis.PlotScatter3(dfref), varargin{:});
        end
