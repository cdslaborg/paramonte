%>  \brief
%>  This is the CascadeLine class for generating
%>  instances of 2-dimensional Line [Cascade visualizations](@ref Cascade)
...
classdef CascadeLine < pm.vis.Cascade
    ...
    methods(Access = public)
        ...
        %>  \FatemehBagheri, May 20 2024, 1:25 PM, NASA Goddard Space Flight Center (GSFC), Washington, D.C.<br>
        %>  \AmirShahmoradi, May 16 2016, 9:03 AM, Oden Institute for Computational Engineering and Sciences (ICES), UT Austin<br>
        function self = CascadeLine(dfref, varargin)
            if nargin < 1
                dfref = [];
            end
            self = self@pm.vis.Cascade(pm.vis.PlotLine(dfref), varargin{:});
        end
