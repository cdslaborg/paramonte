%>  \brief
%>  This is the CascadeHistfit class for generating
%>  instances of 2-dimensional Histfit [Cascade visualizations](@ref Cascade)
...
classdef CascadeHistfit < pm.vis.Cascade
    ...
    methods(Access = public)
        ...
        %>
        %>  \return
        %>  ``self``                :   The output scalar object of class [pm.vis.CascadeHistfit](@ref CascadeHistfit).<br>
        %>
        %>  \interface{CascadeHistfit}
        %>  \code{.m}
        %>
        %>      p = pm.vis.CascadeHistfit(dfref);
        %>      p = pm.vis.CascadeHistfit(dfref, varargin);
        %>
        %>  \endcode
        %>
        %>  \note
        ...
        %>
        %>  \author
        %>  \JoshuaOsborne, May 21 2024, 7:25 AM, University of Texas at Arlington<br>
        %>  \FatemehBagheri, May 20 2024, 1:25 PM, NASA Goddard Space Flight Center (GSFC), Washington, D.C.<br>
        %>  \AmirShahmoradi, May 16 2016, 9:03 AM, Oden Institute for Computational Engineering and Sciences (ICES), UT Austin<br>
        function self = CascadeHistfit(dfref, varargin)
            if nargin < 1
                dfref = [];
            end
            self = self@pm.vis.Cascade(pm.vis.PlotHistfit(dfref), varargin{:});
        end
