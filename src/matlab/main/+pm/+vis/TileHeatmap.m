%>  \brief
%>  This is the [pm.vis.TileHeatmap](@ref TileHeatmap) class for generating
...
classdef TileHeatmap < pm.vis.Tile
    ...
    methods(Access = public)
        ...
        %>
        %>  \return
        %>  ``self``                :   An object of [pm.vis.TileHeatmap](@ref TileHeatmap) class.<br>
        %>
        %>  \interface{TileHeatmap}
        %>  \code{.m}
        %>
        %>      t = pm.vis.TileHeatmap(dfref);
        %>      t = pm.vis.TileHeatmap(dfref, varargin);
        %>
        %>  \endcode
        %>
        %>  \note
        ...
        %>
        %>  \author
        %>  \JoshuaOsborne, May 22 2024, 7:21 PM, University of Texas at Arlington<br>
        %>  \FatemehBagheri, May 20 2024, 1:25 PM, NASA Goddard Space Flight Center (GSFC), Washington, D.C.<br>
        %>  \AmirShahmoradi, May 16 2016, 9:03 AM, Oden Institute for Computational Engineering and Sciences (ICES), UT Austin<br>
        function self = TileHeatmap(dfref, varargin)
            if nargin < 1
                dfref = [];
            end
            self = self@pm.vis.Tile(pm.vis.SubplotHeatmap(dfref), varargin{:});
        end
