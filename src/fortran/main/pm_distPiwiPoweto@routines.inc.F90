...
#error  "Unrecognized interface."
        ...
        CHECK_ASSERTION(__LINE__, alpha(1) > 0._RKC .or. logLimX(1) > -Log_HUGE, SK_"@getPiwiPowetoLogPDFNF(): The conditions `alpha(1) > 0._RKC .or. logLimX(1) > -Log_HUGE` must hold. alpha(1), logLimX(1), -Log_HUGE = "//getStr([alpha(1), logLimX(1), -Log_HUGE]))
        CHECK_ASSERTION(__LINE__, alpha(lenAlpha) < 0._RKC .or. logLimX(lenLogLimX) < Log_HUGE, SK_"@getPiwiPowetoLogPDFNF(): The conditions `alpha(lenAlpha) < 0._RKC .or. logLimX(lenLogLimX) < Log_HUGE` must hold. alpha(lenAlpha), logLimX(lenLogLimX), Log_HUGE = "//getStr([alpha(lenAlpha), logLimX(lenLogLimX), Log_HUGE]))
        ...
        do i = lenAlpha, 1_IK, -1_IK
            ...
            logPDF = logPDFNF(i) + (alpha(i) - 1._RKC) * logx
            return
        end do
        ...
        logPDF = logPDFNF(bin) + (alpha(bin) - 1._RKC) * logx
...
#if     CHECK_ENABLED
        block
            real(RKC) :: logPDFNF_ref(size(logPDFNF, kind = IK)), cumSumArea_ref(size(cumSumArea, kind = IK))
            logPDFNF_ref = getPiwiPowetoLogPDFNF(alpha, logLimX, cumSumArea_ref)
            CHECK_ASSERTION(__LINE__, all(abs(logPDFNF - logPDFNF_ref) <= epsilon(0._RKC) * 1000), SK_"@setPiwiPowetoCDF(): The condition `all(abs(logPDFNF - logPDFNF_ref) <= epsilon(0._RKC) * 1000)` must hold. logPDFNF - logPDFNF_ref = "//getStr([logPDFNF - logPDFNF_ref]))
            CHECK_ASSERTION(__LINE__, all(abs(cumSumArea - cumSumArea_ref) <= epsilon(0._RKC) * 1000), SK_"@setPiwiPowetoCDF(): The condition `all(abs(cumSumArea - cumSumArea_ref) <= epsilon(0._RKC) * 1000)` must hold. cumSumArea - cumSumArea_ref = "//getStr([cumSumArea - cumSumArea_ref]))
        end block
#endif
        cdf = cumSumArea(bin)
        ...
        !CHECK_ASSERTION(__LINE__, logLimX(size(logLimX,1,IK)) < log(huge(logLimX)), SK_"@getPiwiPowetoLogPDFNF(): The condition `logLimX(size(logLimX)) < log(huge(logLimX))` must hold. logLimX = "//getStr(logLimX))
        !CHECK_ASSERTION(__LINE__, merge(alpha(lenAlpha) < -1._RKC, .true., size(logLimX,1,IK) == lenAlpha), SK_"@getPiwiPowetoLogPDFNF(): The condition `alpha(size(alpha)) < -1._RKC` must hold. alpha = "//getStr(alpha))
        do i = lenAlpha, 1_IK, -1_IK
            if (logx < logLimX(i)) cycle
