...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        character(*,SKG), parameter :: NUM_ALPHA = SKG_"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"
#endif
        ...
        integer(IKG) :: i, numlen
        numlen = len(numeral, IKG)
        CHECK_ASSERTION(__LINE__, base > 0_IKG, SK_"@getDecimal(): The input `base` must be positive integer. base = "//getStr(base))
        CHECK_ASSERTION(__LINE__, numlen > 0_IKG, SK_"@getDecimal(): The input `numeral` must be non-empty. len(numeral) = "//getStr(numlen))
        !   \todo
        !   \pvhigh
        !   \warning
        ...
        CHECK_ASSERTION(__LINE__, verify(getStr(numeral), getStr(NUM_ALPHA(1:base)), kind = IKG) == 0_IKG, \
...
#if         __GFORTRAN__
            block
                character(1,SKG) :: numeral_char
                numeral_char = numeral(i:i)
            decimal = decimal + (index(NUM_ALPHA, numeral_char, kind = IKG) - 1_IKG) * base**(numlen - i)
            end block
#else
            decimal = decimal + (index(NUM_ALPHA, numeral(i:i), kind = IKG) - 1_IKG) * base**(numlen - i)
#endif
        end do
        ...
        integer(IKG) :: remainderPlusOne, lamiced
        integer(IK) :: i, numlen
        CHECK_ASSERTION(__LINE__, range(0_IKG) < precision(0._RK64), \
        SK_"@getNumeral(): Hello, the distant future user. The condition `range(0_IKG) < precision(0._RK64)` must hold. This is an internal library limitation. Please report this to the developers if they are still alive.")
        CHECK_ASSERTION(__LINE__, base > 0_IKG, SK_"@getNumeral(): The input `base` must be a positive number. base = "//getStr(base))
        CHECK_ASSERTION(__LINE__, decimal > 0_IKG, SK_"@getNumeral(): The input `decimal` must be a positive number. decimal = "//getStr(decimal))
        numlen = int(log10(real(huge(1_IKG),RK64)) / log10(real(base,RK64)), IK) + 1_IK
        allocate(character(numlen,SKG) :: numeral)
        lamiced = decimal
        do i = numlen, 1_IK, -1_IK
            ...
            remainderPlusOne = mod(lamiced, base) + 1_IKG
