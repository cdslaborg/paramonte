...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        CHECK_ASSERTION(__LINE__, 0._RKC < probSuccess, SK_"@getGeomCyclicLogPMF(): The condition `0 <= probSuccess` must hold. probSuccess = "//getStr(probSuccess))
        CHECK_ASSERTION(__LINE__, probSuccess <= 1._RKC, SK_"@getGeomCyclicLogPMF(): The condition `probSuccess <= 1` must hold. probSuccess = "//getStr(probSuccess))
        ...
        real(RKC), parameter :: logProbSuccessMin = log(epsilon(0._RKC))
        real(RKC), parameter :: NEGINF = -log(huge(logPMF))
        real(RKC) :: logDenominator
#if     Def_ENABLED
        real(RKC) :: logProbFailure
#endif
        ! Validate the input.
        ...
        CHECK_ASSERTION(__LINE__, all([stepSuccess] <= period), SK_"@setGeomCyclicLogPMF(): The condition `all([stepSuccess] <= period)` must hold. stepSuccess, period = "//getStr([real(RKC) :: stepSuccess, period]))
        CHECK_ASSERTION(__LINE__, logProbSuccess <= 0._RKC, SK_"@setGeomCyclicLogPMF(): The condition `logProbSuccess <= 0.` must hold. logProbSuccess = "//getStr(logProbSuccess))
        ! Compute the PMF.
        if (logProbSuccess < 0._RKC) then ! imperfect probability of success.
            if (logProbSuccessMin < logProbSuccess) then
#if             Log_ENABLED
                CHECK_ASSERTION(__LINE__, abs(1._RKC - exp(logProbSuccess) - exp(logProbFailure)) < epsilon(0._RKC) * 100, SK_"@setGeomCyclicLogPMF(): The condition `exp(logProbFailure) + exp(logProbSuccess) == 1.` must hold. logProbFailure, logProbSuccess = "//getStr([logProbFailure, logProbSuccess]))
#elif           Def_ENABLED
...
#error          "Unrecognized interface."
        ...
        CHECK_ASSERTION(__LINE__, 0._RKC < probSuccess, SK_"@setGeomCyclicLogCDF(): The condition `0 <= probSuccess` must hold. probSuccess = "//getStr(probSuccess))
        CHECK_ASSERTION(__LINE__, probSuccess <= 1._RKC, SK_"@setGeomCyclicLogCDF(): The condition `probSuccess <= 1` must hold. probSuccess = "//getStr(probSuccess))
...
#error      "Unrecognized interface."
        ...
        CHECK_ASSERTION(__LINE__, 0._RKC < probSuccess, SK_"@getGeomCyclicRand(): The condition `0 <= probSuccess` must hold. probSuccess = "//getStr(probSuccess))
        if (probSuccess < 1._RKC) then
            call setGeomCyclicRand(rand, log(1._RKC - probSuccess), period)
        else
            rand = 1_IK
            CHECK_ASSERTION(__LINE__, 0_IK < period, SK_"@getGeomCyclicRand(): The condition `0 < period` must hold. probSuccess = "//getStr(period))
            CHECK_ASSERTION(__LINE__, probSuccess <= 1._RKC, SK_"@getGeomCyclicRand(): The condition `probSuccess <= 1` must hold. probSuccess = "//getStr(probSuccess))
        end if
...
#error      "Unrecognized interface."
        ...
        real(RKC) :: sumFreq
        real(RKC) :: logProbFailure
        real(RKC) :: logProbSuccess
        real(RKC) :: diff(size(stepSuccess, 1, IK))
        real(RKC) :: logFreqSuccess(size(freqSuccess, 1, IK))
        real(RKC), parameter :: small = sqrt(epsilon(0._RKC))
        integer(IK), parameter :: niter_def = int(1000 * precision(0._RKC) / 53., IK)
        ...
        CHECK_ASSERTION(__LINE__, maxval(stepSuccess) <= period, SK_"@isFailedGeomCyclicFit(): The condition `maxval(stepSuccess) <= period` must hold. maxval(stepSuccess), period = "//getStr([real(RKC) :: maxval(stepSuccess), period]))
        ...
        !    probSuccess = xmin(1)
        !end block
        probSuccess = getMinBrent(getNegLogLike, xlow = small, xupp = 1._RKC - small, niter = niter)
        failed = niter_def < niter
        if (failed) return

        ! Find the normalization factor.

        logProbFailure = log(1._RKC - probSuccess)
        logProbSuccess = log(probSuccess)

        niter = niter_def
        call setGeomCyclicLogPMF(diff, stepSuccess, logProbSuccess, logProbFailure, period)
        logFreqSuccess = log(real(freqSuccess, RKC))
    ...
    contains
        ...
        PURE function getNegLogLike(probSuccess) result(negLogLike)
            real(RKC), intent(in) :: probSuccess!(1)
            real(RKC) :: negLogLike
            if (0._RKC < probSuccess .and. probSuccess < 1._RKC) then
        ...
        do i = 1, size(stepSuccess, 1, IK)
            if (0 < freqSuccess(i)) then
                ...
                logFreqSuccess(counter) = log(real(freqSuccess(counter), RKC))
        ...
        real(RKC) :: successProbFisherTransLogNormFac(2) ! vector of the two parameters to fit.
        real(RKC), parameter :: SUCCESS_PROB_INIT_GUESS = 0.23_RKC
        real(RKC), parameter :: FISHER_TRANS_SUCCESS_PROB_INIT_GUESS = atanh(2 * (SUCCESS_PROB_INIT_GUESS - 0.5_RKC))
        CHECK_ASSERTION(__LINE__, all(0_IK < stepSuccess), SK_"@isFailedGeomCyclicFit(): The condition `all(0 < stepSuccess)` must hold. stepSuccess = "//getStr(stepSuccess))
        CHECK_ASSERTION(__LINE__, all(0._RKC <= logFreqSuccess), SK_"@isFailedGeomCyclicFit(): The condition `all(0 < stepSuccess)` must hold. stepSuccess = "//getStr(logFreqSuccess))
        CHECK_ASSERTION(__LINE__, 1_IK < size(stepSuccess, 1, IK), SK_"@isFailedGeomCyclicFit(): The condition `1 < size(stepSuccess)` must hold. size(stepSuccess) = "//getStr(size(stepSuccess, 1, IK)))
        CHECK_ASSERTION(__LINE__, maxval(stepSuccess) <= period, SK_"@isFailedGeomCyclicFit(): The condition `maxval(stepSuccess) <= period` must hold. maxval(stepSuccess), period = "//getStr([real(RKC) :: maxval(stepSuccess), period]))
        CHECK_ASSERTION(__LINE__, size(stepSuccess, 1, IK) <= period, SK_"@isFailedGeomCyclicFit(): The condition `size(stepSuccess) <= period` must hold. size(stepSuccess), period = "//getStr([size(stepSuccess, 1, IK), period]))
        ...
        successProbFisherTransLogNormFac(2) = 0._RKC
        ...
        if (.not. failed) then
            probSuccess = 0.5_RKC * tanh(successProbFisherTransLogNormFac(1)) + 0.5_RKC ! reverse Fisher-transform
    ...
    contains
        ...
        PURE function getSumDistSq(successProbFisherTransLogNormFac) result(sumDistSq)
            real(RKC), intent(in) :: successProbFisherTransLogNormFac(2)
            real(RKC) :: logPMF(size(stepSuccess, 1, IK)), logProbSuccess, sumDistSq
            logProbSuccess = log(0.5_RKC * tanh(successProbFisherTransLogNormFac(1)) + 0.5_RKC)
