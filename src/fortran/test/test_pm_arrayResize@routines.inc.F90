...
!>  \brief
!>  This module contains implementations of the tests of the procedures under the generic interfaces of [pm_arrayResize](@ref pm_arrayResize).
!>
!>  \final
!>
!>  \author
...
#if     setResized_D1_SK_ENABLED || setResized_D2_SK_ENABLED || setResized_D3_SK_ENABLED
#define TYPE_KIND character(2,SKC) ::
#else
#define TYPE_KIND
#endif

#if     setResized_D1_LK_ENABLED || setResized_D2_LK_ENABLED || setResized_D3_LK_ENABLED
#define IS_EQUAL .eqv.
#else
#define IS_EQUAL ==
#endif

#if     setResized_D0_ENABLED || setResized_D1_ENABLED
#define SET_BND(X,lb,ub) X(lb : ub)
#define SET_DIM(X) X(:)
#define SET_SIZE(X) X
#elif   setResized_D2_ENABLED
#define SET_BND(X,lb,ub) X(lb(1) : ub(1), lb(2) : ub(2))
#define SET_SIZE(X) X(rank(array))
#define SET_DIM(X) X(:,:)
#elif   setResized_D3_ENABLED
...
#elif   setResized_D1_IK_ENABLED || setResized_D2_IK_ENABLED || setResized_D3_IK_ENABLED
...
#elif   setResized_D1_LK_ENABLED || setResized_D2_LK_ENABLED || setResized_D3_LK_ENABLED
...
#elif   setResized_D1_CK_ENABLED || setResized_D2_CK_ENABLED || setResized_D3_CK_ENABLED
...
#elif   setResized_D1_RK_ENABLED || setResized_D2_RK_ENABLED || setResized_D3_RK_ENABLED
...
#error  "Unrecognized interface."
    ...
    contains
        ...
        subroutine runTestsWithUnalloc(failed, errmsg)
            ...
            call setResized(array, ub - lb + 1_IK, failed, errmsg)
        ...
        subroutine runTestsWith(failed, errmsg)
            ...
            if (all([lb] <= [lbold]) .and. all([ubold] <= [ub]) .and. getUnifRand()) then
                if (assumedSize) then
                    ...
                    !write(*,*) "lb, ub", lb, ub
                    call setResized(array, failed, errmsg)
                ...
                allocate(array, source = arrayInit)
                call setCoreHalo(array_ref, array, fill, lbc - lb)
                call setResized(array, ub - lb + 1_IK, lbc, failed, errmsg)
                call report(__LINE__, failed, errmsg)
            end if
            ...
            call setResized(array, ub - lb + 1_IK, lbc, lbcold, ubcold, failed, errmsg)
            call report(__LINE__, failed, errmsg)
...
#endif
            call display()
            call test%assert(assertion, SK_"Call to setResized() must correctly rebind and refill `array` with present(failed), present(errmsg) = "//getStr([present(failed), present(errmsg)]), int(line, IK))
        end subroutine
