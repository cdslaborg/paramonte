...
!>  \brief
!>  This module contains implementations of the tests of the procedures under the generic interfaces of [pm_arrayRebill](@ref pm_arrayRebill).
!>
!>  \final
!>
!>  \author
...
#if     setRebilled_D1_LK_ENABLED || setRebilled_D2_LK_ENABLED || setRebilled_D3_LK_ENABLED
#define IS_EQUAL .eqv.
#else
#define IS_EQUAL ==
#endif

#if     setRebilled_D1_ENABLED
#define SET_BND(X,lb,ub) X(lb : ub)
#define SET_DIM(X) X(:)
#define SET_SIZE(X) X
#define ALL
#elif   setRebilled_D2_ENABLED
#define SET_BND(X,lb,ub) X(lb(1) : ub(1), lb(2) : ub(2))
#define SET_SIZE(X) X(rank(array))
#define SET_DIM(X) X(:,:)
#elif   setRebilled_D3_ENABLED
...
#error  "Unrecognized interface."
    ...
    contains
        ...
        subroutine runTestsWithUnalloc(failed, errmsg)
            ...
            call setRebilled(array, fill, lb, ub, failed, errmsg)
        ...
        subroutine runTestsWith(failed, errmsg)
            ...
            if (ALL(lb <= lbold) .and. ALL(ubold <= ub) .and. getUnifRand()) then
                ...
                allocate(array, source = arrayInit)
                call setCoreHalo(array_ref, array, fill, lbc - lb)
                call setRebilled(array, fill, lb, ub, lbc, failed, errmsg)
                call report(__LINE__, failed, errmsg)
            end if
            ...
            call setRebilled(array, fill, lb, ub, lbc, lbcold, ubcold, failed, errmsg)
            call report(__LINE__, failed, errmsg)
        ...
        subroutine report(line, failed, errmsg)
            ...
            assertion = assertion .and. all(array IS_EQUAL array_ref)
            call display()
            call test%assert(assertion, SK_"Call to setRebilled() must correctly rebind and refill `array` with present(failed), present(errmsg) = "//getStr([present(failed), present(errmsg)]), int(line, IK))
        end subroutine
