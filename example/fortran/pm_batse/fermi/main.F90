program fermi
    ...
    real(RKC), parameter :: ZMIN = 0.1_RKC, ZMAX = 1.01e2_RKC
    real(RKC), parameter :: LOGZPLUS1_MIN = log(ZMIN + 1._RKC), LOGZPLUS1Z_MAX = log(ZMAX + 1._RKC)
    real(RKC), parameter :: DELTA_LOGZPLUS1 = (LOGZPLUS1Z_MAX - LOGZPLUS1_MIN) / (nred - 1)
    real(RKC), parameter :: TIME_DILATION_EXPO = 1._RKC ! kfacor correction. .66 is the other possibility.
    real(RKC), parameter :: LBNEW = .1_RKC, UBNEW = 20000._RKC, LB = 10._RKC, UB = 1000._RKC ! kev unit.
    real(RKC), parameter :: ALPHA = -1.1_RKC, BETA = -2.3_RKC
    real(RKC), allocatable :: grbz(:,:) ! (1:nvar, size(zindex) pbol, epk, sbol, t90
    real(RKC), allocatable :: logz(:) ! log of known z values.
    ...
    real(RKC), parameter :: LN10 = log(10._RKC)
    real(RKC), parameter :: LOG_FULL_SKY_SOLID_ANGLE = log(4._RKC * PI)



    type :: zinfo_type
        real(RKC) :: z, logSFR
        real(RKC) :: logInt2Obs(nvar) ! the redshift mapping: the subtraction of the log GRB attributes in rest frame from observer frame.
    end type
    type(zinfo_type) :: zinfo(nred)



    real(RKC), allocatable :: grb(:,:) ! pbol, epk, sbol, t90



    type :: model_type
        real(RKC) :: avg(nvar)
        real(RKC) :: invcov(nvar, nvar)
    end type
    ...
    model%invcov = reshape( [ +21.183484566160494_RKC, +7.095003864339406_RKC, -14.827122427564412_RKC, -0.310607307395483_RKC &
                            , +7.0950038643394060_RKC, +4.573466054775477_RKC, -5.4639334908475360_RKC, -0.193710749126834_RKC &
                            , -14.827122427564412_RKC, -5.463933490847536_RKC, +10.954404830891077_RKC, -0.415177537893988_RKC &
                            ,  -0.310607307395483_RKC, -0.193710749126834_RKC, -0.4151775378939880_RKC, +1.997104942888680_RKC &
                            ], shape = [nvar, nvar])
    ...
    !block
    !    real(RKC) :: std(nvar), rho(nvar*(nvar + 1)/2), cor(nvar, nvar), cov(nvar, nvar)
    ...
    block
        real(RKC) :: zplus1, logzplus1, logLumDisCM, disComTransNormed
        open(newunit = unit, file = SK_"zgrid.txt", status = "replace")
        write(unit,"(*(g0))") "z,logLumDisCM,logSFR"
        do ired = 1, nred
            logzplus1 = LOGZPLUS1_MIN + real(ired - 1, RKC) * DELTA_LOGZPLUS1
            zplus1 = exp(logzplus1)
            zinfo(ired)%z = zplus1 - 1._RKC
    ...
    if(shiftEnabled) THEN
        do igrb=1,size(grb(1,:))
            grb(1:nvar,igrb) = grb(1:nvar,igrb) + [-0.118033988749895_RKC,3.61803398874989_RKC,1.50000000000000000_RKC,0.881966011250105_RKC]
        end do
        ![0.0000000000000000_RKC, 0.0000000000000000_RKC, 0.0000000000000000_RKC, -0.23606797749978969_RKC]!0.0000000000000000_RKC        0.0000000000000000_RKC        0.0000000000000000_RKC      -0.23606797749978969_RKC
        !clean shift -0.11803398874989479_RKC,1.5000000000000000_RKC,0.50000000000000000_RKC,0.50000000000000000_RKC
    end if
    ...
    block
        ...
        real(RKC) :: zsum(2, ngrb)  ! table of (zmode, zreal) for all events.
        real(RKC) :: zpdf(nred, 2)  ! table of (z, zpdf) for individual events.
        do igrb = 1, ngrb
    ...
    block
        ...
        real(RKC) :: optimalShift(nvar) ! pbol, epk, sbol, t90 : corrections to add to GRB attributes to make predictions optimal.
        ...
        optimalShift = [-0.118033988749895_RKC,3.61803398874989_RKC,1.50000000000000000_RKC,0.881966011250105_RKC]!0.5_RKC
        !print *,grbz(1:nvar, :)
...
contains
    ...
    !    implicit none
    !    real(RKC), intent(in) :: grbScale(nvar) ! pbol, epk, sbol, t90 : scaling corrections to add to GRB attributes to make predictions optimal.
    !    real(RKC) :: errSq
    !    integer(IK) :: igrbz, maxLocPDF
    !    real(RKC) :: grbzScaled(nvar, ngrbz), zpdf(nred) ! table of zpdf for individual z-known events.
    ...
    function getErrSq(grbShift) result(errSq)
        ...
        implicit none
        real(RKC), intent(in) :: grbShift(nvar) ! pbol, epk, sbol, t90 : corrections to add to GRB attributes to make predictions optimal.
        real(RKC) :: errSq, grbzScaled(nvar,ngrbz),grbScale(nvar)
        integer(IK) :: igrbz, maxLocPDF
        real(RKC) :: grbzShifted(nvar, ngrbz), zpdf(nred) ! table of zpdf for individual z-known events.
    ...
    pure subroutine setRedshiftPDF(pdf, maxLocPDF, zinfo, event)
        ...
        real(RKC)       , intent(in)    , contiguous    :: event(:)
        real(RKC)       , intent(out)   , contiguous    :: pdf(:)
        integer(IK)     , intent(out)                   :: maxLocPDF
        real(RKC)                                       :: grbIntShifted(size(event)) ! shifted w.r.t. the mean of the model.
        integer(IK)                                     :: ired
        ...
        do ired = 1, size(zinfo)
            ...
            pdf(ired) = zinfo(ired)%logSFR - 0.5_RKC * dot_product(grbIntShifted, matmul(model%invcov, grbIntShifted))
        end do
