program example
    ...
    block
        ...
        call disp%show("cdf")
        call disp%show( cdf )
        call disp%show("call setKolmQuan(x, cdf)")
                        call setKolmQuan(x, cdf)
        call disp%show("x")
        call disp%show( x )
        call disp%skip()
    end block
    ...
    block
        ...
        call setKolmQuan(cdfx(:, 2), cdfx(:, 1))
        if (0 /= getErrTableWrite(SK_"setKolmQuan.RK.txt", cdfx, header = SK_"cdf,quantile")) error stop "table output failed."
    end block
