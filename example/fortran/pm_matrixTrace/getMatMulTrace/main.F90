program example
    ...
    block
        ...
        do itry = 1, ntry
            ...
            call disp%show("mat")
            call disp%show( mat )
            call disp%show("getMatMulTrace(mat)")
            call disp%show( getMatMulTrace(mat) )
            call disp%show("getMatMulTrace(getMatCopy(rdpack, mat, rdpack, subset = uppDia))")
            call disp%show( getMatMulTrace(getMatCopy(rdpack, mat, rdpack, subset = uppDia)) )
            call disp%show("getMatMulTrace(getMatCopy(rdpack, mat, rdpack, subset = lowDia))")
            call disp%show( getMatMulTrace(getMatCopy(rdpack, mat, rdpack, subset = lowDia)) )
            call disp%show("getMatMulTrace(getMatCopy(lfpack, mat, rdpack, lowDia), lfpack, lowDia)")
            call disp%show( getMatMulTrace(getMatCopy(lfpack, mat, rdpack, lowDia), lfpack, lowDia) )
            call disp%skip()
        end do
    end block
