program example
    ...
    block
        ...
        if (0 /= getErrTableWrite(SK_"xoshiro256ssw_type.IK.txt", rand)) error stop "Table writing failed."
    end block

    block
        complex :: rand(5000)
        call setUnifRand(rng, rand, (-2., +2.), (3., 5.))
        if (0 /= getErrTableWrite(SK_"xoshiro256ssw_type.CK.txt", rand)) error stop "Table writing failed."
    end block

    block
        real :: rand(5000)
        call setUnifRand(rng, rand)
        if (0 /= getErrTableWrite(SK_"xoshiro256ssw_type.RK.txt", rand)) error stop "Table writing failed."
    end block
