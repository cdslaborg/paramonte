program example
    ...
    block
        type(stop_type) :: stop
        call disp%show("stop = stop_type(unit = disp%unit, newline = SK_'\n', prefix = SK_'    ParaMonte', renabled = .true._LK)")
                        stop = stop_type(unit = disp%unit, newline = SK_'\n', prefix = SK_'    ParaMonte', renabled = .true._LK)
        call disp%show("str")
        call disp%show( str, deliml = SK_"""" )
        call disp%skip()
        call disp%show("call stop%show(str)")
                        call stop%show(str)
        call disp%skip()
        call disp%show("call stop%show(str, width = 72_IK, sticky = .true._LK)")
                        call stop%show(str, width = 72_IK, sticky = .true._LK)
        call disp%skip()
        call disp%show("call stop%show(str) ! same width as before.")
