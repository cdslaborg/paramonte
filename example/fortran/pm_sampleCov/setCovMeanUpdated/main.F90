program example
    ...
    block
        ...
        do itry = 1, ntry
            ...
            call disp%show("mean")
            call disp%show( mean )
            call disp%show("call setCovMeanUpdated(covA, meanA, meanB, real(nsamA, TKC) / real(nsamA + nsamB, TKC), uppDia)")
                            call setCovMeanUpdated(covA, meanA, meanB, real(nsamA, TKC) / real(nsamA + nsamB, TKC), uppDia)
            call disp%show("meanA")
            call disp%show( meanA )
            call disp%show("mean ! reference")
            call disp%show( mean )
            call disp%show("covA")
            call disp%show( covA )
