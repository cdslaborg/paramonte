program example
    ...
    integer(IK) :: rsize, nsam, itry, ntry = 10
    ...
    block
        ...
        block
            ...
            character(:,TKC), allocatable :: array, arref
            integer(IK), allocatable :: weight(:), weiref(:)
            integer(IK) :: skip
            do itry = 1, ntry
                ...
                call disp%show("arref = array")
                                arref = array
                call disp%show("weiref = weight")
                                weiref = weight
                call disp%show("call setRefined(arref, weiref, skip, rsize)")
                                call setRefined(arref, weiref, skip, rsize)
                call disp%show("rsize")
                call disp%show( rsize )
                call disp%show("weiref(1:rsize)")
                call disp%show( weiref(1:rsize) )
    ...
    block
        ...
        block
            ...
            do itry = 1, ntry
                ...
                call disp%show("weiref = weight")
                                weiref = weight
                call disp%show("call setRefined(arref, dim, weiref, skip, rsize)")
                ...
                call disp%show( weiref(1:rsize) )
                call disp%show("arref(:, 1:rsize)")
                call disp%show( arref(:, 1:rsize) , deliml = TKC_"""" )
                call disp%show("weiref = weight")
                                weiref = weight
                call disp%show("arref = transpose(array)")
                                arref = transpose(array)
                call disp%show("call setRefined(arref, 3_IK - dim, weiref, skip, rsize)")
