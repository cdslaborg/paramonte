program example
    ...
    use pm_quadpack, only: getQuadErr, GK21
    ...
    call disp%show("err = getQuadErr(getNormPDF, lb, ub, ABSTOL, RELTOL, GK21, integral, abserr, sinfo, sindex, neval, nint)")
                    err = getQuadErr(getNormPDF, lb, ub, ABSTOL, RELTOL, GK21, integral, abserr, sinfo, sindex, neval, nint)
    ...
    call disp%show("err = getQuadErr(getLogNormPDF, lb, ub, ABSTOL, RELTOL, GK21, integral, abserr, sinfo, sindex, neval, nint)")
                    err = getQuadErr(getLogNormPDF, lb, ub, ABSTOL, RELTOL, GK21, integral, abserr, sinfo, sindex, neval, nint)
    call disp%show("if (err /= 0_IK) error stop 'integration failed: err = '//getStr(err)")
                    if (err /= 0_IK) error stop 'integration failed: err = '//getStr(err)
