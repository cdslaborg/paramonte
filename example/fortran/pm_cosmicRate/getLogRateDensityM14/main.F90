program example
    ...
    call disp%skip()
    call disp%show("!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    call disp%show("!Compute the log(RateDensity) according to the M14 LGRB rate density parameters for the Hopkins and Beacom (2006) SFR model.")
    call disp%show("!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    call disp%skip()
    ...
    call disp%show( redshift )
    call disp%show("getLogRateDensityM14(redshift + 1, log(redshift + 1))")
    call disp%show( getLogRateDensityM14(redshift + 1, log(redshift + 1)) )
    call disp%skip()
    ...
    block
        ...
        rateDensityFormLGRB = exp(getLogRateDensityM14(zplus1, logzplus1))
        ...
        open(newunit = fileUnit, file = "getLogRateDensityM14.csv")
    ...
    block
        ...
        type(err_type) :: err
        sampler%outputFileName = "./zdistM14"
...
contains
    ...
    recursive function getLogFunc(redshift) result(logRateDensity)
        ...
        real(RKG) :: zplus1
        zplus1 = redshift(1) + 1
        logRateDensity = getLogRateDensityM14(zplus1, log(zplus1)) + log(getVolComDiffNormed(zplus1, reltol = sqrt(epsilon(0._RKG))))
    end function
