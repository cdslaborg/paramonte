program example
    ...
    call disp%skip()
    call disp%show("!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    call disp%show("!Compute the log(RateDensity) according to the B10 LGRB rate density parameters for the Hopkins and Beacom (2006) SFR model.")
    call disp%show("!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    call disp%skip()
    ...
    call disp%show( redshift )
    call disp%show("getLogRateDensityB10(log(redshift + 1))")
    call disp%show( getLogRateDensityB10(log(redshift + 1)) )
    call disp%skip()
    ...
    block
        ...
        rateDensityFormLGRB = exp(getLogRateDensityB10(logzplus1))
        ...
        open(newunit = fileUnit, file = "getLogRateDensityB10.csv")
    ...
    block
        ...
        type(err_type) :: err
        sampler%outputFileName = "./zdistB10"
...
contains
    ...
    recursive function getLogFunc(redshift) result(logRateDensity)
        ...
        real(RKC) :: zplus1
        zplus1 = redshift(1) + 1
        logRateDensity = getLogRateDensityB10(log(zplus1)) + log(getVolComDiffNormed(zplus1, reltol = sqrt(epsilon(0._RKC))))
    end function
