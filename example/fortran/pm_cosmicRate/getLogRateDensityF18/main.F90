program example
    ...
    call disp%skip()
    call disp%show("!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    call disp%show("!Compute the log(RateDensity) according to the F18 LGRB rate density parameters for the Hopkins and Beacom (2006) SFR model.")
    call disp%show("!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    call disp%skip()
    ...
    call disp%show( redshift )
    call disp%show("getLogRateDensityF18(redshift + 1, log(redshift + 1))")
    call disp%show( getLogRateDensityF18(redshift + 1, log(redshift + 1)) )
    call disp%skip()
    ...
    block
        ...
        rateDensityFormLGRB = exp(getLogRateDensityF18(zplus1, logzplus1))
        ...
        open(newunit = fileUnit, file = "getLogRateDensityF18.csv")
    ...
    block
        ...
        type(err_type) :: err
        sampler%outputFileName = "./zdistF18"
...
contains
    ...
    recursive function getLogFunc(redshift) result(logRateDensity)
        ...
        real(RKC) :: zplus1
        zplus1 = redshift(1) + 1
        logRateDensity = getLogRateDensityF18(zplus1, log(zplus1)) + log(getVolComDiffNormed(zplus1, reltol = sqrt(epsilon(0._RKC))))
    end function
