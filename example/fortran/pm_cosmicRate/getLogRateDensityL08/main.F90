program example
    ...
    call disp%skip()
    call disp%show("!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    call disp%show("!Compute the log(RateDensity) according to the L08 LGRB rate density parameters for the Hopkins and Beacom (2006) SFR model.")
    call disp%show("!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    call disp%skip()
    ...
    call disp%show( redshift )
    call disp%show("getLogRateDensityL08(log(redshift + 1))")
    call disp%show( getLogRateDensityL08(log(redshift + 1)) )
    call disp%skip()
    ...
    block
        ...
        rateDensityFormLGRB = exp(getLogRateDensityL08(logzplus1))
        ...
        open(newunit = fileUnit, file = "getLogRateDensityL08.csv")
    ...
    block
        ...
        type(err_type) :: err
        sampler%outputFileName = "./zdistL08"
...
contains
    ...
    recursive function getLogFunc(redshift) result(logRateDensity)
        ...
        real(RKC) :: zplus1
        zplus1 = redshift(1) + 1
        logRateDensity = getLogRateDensityL08(log(zplus1)) + log(getVolComDiffNormed(zplus1, reltol = sqrt(epsilon(0._RKC))))
    end function
