program example
    ...
    block
        ...
        do itry = 1, ntry
            ...
            call disp%show("call setVarMeanMerged(varMerged, meanMerged, var(2), mean(2), var(1), mean(1), ub(1) / real(ub(2), TKC))")
                            call setVarMeanMerged(varMerged, meanMerged, var(2), mean(2), var(1), mean(1), ub(1) / real(ub(2), TKC))
            call disp%show("meanMerged")
            call disp%show( meanMerged )
            call disp%show("varMerged")
            call disp%show( varMerged )
            call disp%show("call setVarMeanMerged(var(2), mean(2), var(1), mean(1), ub(1) / real(ub(2), TKC))")
                            call setVarMeanMerged(var(2), mean(2), var(1), mean(1), ub(1) / real(ub(2), TKC))
    ...
    block
        complex(TKC) :: mean(0:nsam), meanMerged
        ...
        do itry = 1, ntry
            ...
            call disp%show("ub")
            call disp%show( ub )
            call disp%show("sample = cmplx(getUnifRand(-1., +1., ub(nsam)), getUnifRand(-1., +1., ub(nsam)), TKC)")
                            sample = cmplx(getUnifRand(-1., +1., ub(nsam)), getUnifRand(-1., +1., ub(nsam)), TKC)
            call disp%show("sample")
            call disp%show( sample )
            call disp%show("mean(0) = getMean(sample)")
                            mean(0) = getMean(sample)
    ...
    block
        ...
        do itry = 1, ntry
            ...
            call disp%show("call setVarMeanMerged(varMerged, meanMerged, var(2), mean(2), var(1), mean(1), real(sum(iweight(:ub(1))), TKC) / real(sum(iweight), TKC))")
                            call setVarMeanMerged(varMerged, meanMerged, var(2), mean(2), var(1), mean(1), real(sum(iweight(:ub(1))), TKC) / real(sum(iweight), TKC))
            call disp%show("meanMerged")
            call disp%show( meanMerged )
            call disp%show("varMerged")
            call disp%show( varMerged )
            call disp%show("call setVarMeanMerged(var(2), mean(2), var(1), mean(1), real(sum(iweight(:ub(1))), TKC) / real(sum(iweight), TKC))")
                            call setVarMeanMerged(var(2), mean(2), var(1), mean(1), real(sum(iweight(:ub(1))), TKC) / real(sum(iweight), TKC))
            call disp%show("mean(2)")
            call disp%show( mean(2) )
            call disp%show("mean(0) ! reference")
            call disp%show( mean(0) )
    ...
    block
        ...
        do itry = 1, ntry
            ...
            call disp%show("call setVarMeanMerged(varMerged, meanMerged, var(2), mean(2), var(1), mean(1), real(sum(rweight(:ub(1))), TKC) / real(sum(rweight), TKC))")
                            call setVarMeanMerged(varMerged, meanMerged, var(2), mean(2), var(1), mean(1), real(sum(rweight(:ub(1))), TKC) / real(sum(rweight), TKC))
            call disp%show("meanMerged")
            call disp%show( meanMerged )
            call disp%show("varMerged")
            call disp%show( varMerged )
            call disp%show("call setVarMeanMerged(var(2), mean(2), var(1), mean(1), real(sum(rweight(:ub(1))), TKC) / real(sum(rweight), TKC))")
                            call setVarMeanMerged(var(2), mean(2), var(1), mean(1), real(sum(rweight(:ub(1))), TKC) / real(sum(rweight), TKC))
            call disp%show("mean(2)")
            call disp%show( mean(2) )
            call disp%show("mean(0) ! reference")
            call disp%show( mean(0) )
    ...
    block
        ...
        do itry = 1, ntry
            ...
            call disp%show("call setVarMeanMerged(varMerged, meanMerged, var(:,2), mean(:,2), var(:,1), mean(:,1), real(ub(1), TKC) / real(ub(2), TKC))")
                            call setVarMeanMerged(varMerged, meanMerged, var(:,2), mean(:,2), var(:,1), mean(:,1), real(ub(1), TKC) / real(ub(2), TKC))
            call disp%show("meanMerged")
            call disp%show( meanMerged )
            call disp%show("varMerged")
            call disp%show( varMerged )
            call disp%show("call setVarMeanMerged(var(:,2), mean(:,2), var(:,1), mean(:,1), real(ub(1), TKC) / real(ub(2), TKC))")
                            call setVarMeanMerged(var(:,2), mean(:,2), var(:,1), mean(:,1), real(ub(1), TKC) / real(ub(2), TKC))
            call disp%show("mean(:,2)")
            call disp%show( mean(:,2) )
            call disp%show("mean(:,0) ! reference")
            call disp%show( mean(:,0) )
    ...
    block
        complex(TKC), allocatable :: sample(:,:)
        complex(TKC), allocatable :: mean(:,:), meanMerged(:)
        real(TKC), allocatable :: var(:,:), varMerged(:)
        do itry = 1, ntry
            call disp%skip()
            call disp%show("dim = 2; lb(1) = 1; ub(1) = getUnifRand(2, 7)")
