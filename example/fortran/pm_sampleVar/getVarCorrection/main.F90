program example
    ...
    block
        ...
        integer(RKG), allocatable :: weight(:)
        real(RKG) :: correction
        do itry = 1, ntry
            call disp%skip()
            ...
            call disp%show( weight )
            call disp%show("correction = getVarCorrection(real(sum(weight), RKG))")
                            correction = getVarCorrection(real(sum(weight), RKG))
            call disp%show("correction")
            call disp%show( correction )
            call disp%show("correction = getVarCorrection(real(sum(weight), RKG), real(sum(weight**2), RKG))")
    ...
    block
        ...
        real(RKG), allocatable :: sampleSize(:)
        sampleSize = getLogSpace(log(1.1_RKG), log(1001._RKG), 1000_IK)
