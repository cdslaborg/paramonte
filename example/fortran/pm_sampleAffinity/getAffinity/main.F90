program example
    ...
    block
        ...
        do itry = 1, ntry
            ...
            call disp%show( tform )
            call disp%show("affinity = getAffinity(sample, dim, tform) ! whole sample transformation.")
            ...
            call disp%show("    affinity(:,isam) = getAffinity(sample(:,isam), tform) ! point-wise transformation")
            ...
            call disp%show("transpose(affinity)")
            call disp%show( transpose(affinity) )
            call disp%show("affinInv = getAffinity(affinity, dim, getMatInv(tform)) ! inverse transformation.")
            ...
            call disp%show("tform")
            call disp%show( tform )
            call disp%show("affinity = getAffinity(sample, dim, tform, class = upperDiag) ! whole sample transformation.")
                            affinity = getAffinity(sample, dim, tform, class = upperDiag) ! whole sample transformation.
            call disp%show("transpose(affinity)")
            call disp%show( transpose(affinity) )
            call disp%show("do isam = 1, nsam")
            call disp%show("    affinity(:,isam) = getAffinity(sample(:,isam), tform, upperDiag) ! point-wise transformation")
            call disp%show("end do")
                            do isam = 1, nsam
            ...
            call disp%show("transpose(affinity)")
            call disp%show( transpose(affinity) )
            call disp%show("affinInv = getAffinity(affinity, dim, getMatInv(tform, upperDiag), class = upperDiag) ! inverse transformation.")
            ...
            call disp%show( transpose(sample) )
            call disp%skip()
            call disp%show("affinity = getAffinity(sample, dim, tform, class = upperUnit) ! whole sample transformation.")
                            affinity = getAffinity(sample, dim, tform, class = upperUnit) ! whole sample transformation.
            call disp%show("transpose(affinity)")
            call disp%show( transpose(affinity) )
            call disp%show("do isam = 1, nsam")
            call disp%show("    affinity(:,isam) = getAffinity(sample(:,isam), tform, upperUnit) ! point-wise transformation")
            call disp%show("end do")
                            do isam = 1, nsam
            ...
            call disp%show("transpose(affinity)")
            call disp%show( transpose(affinity) )
            call disp%show("affinInv = getAffinity(affinity, dim, getMatInv(tform), class = upperUnit) ! inverse transformation.")
            ...
            call disp%show( transpose(sample) )
            call disp%skip()
            call disp%show("affinity = getAffinity(sample, dim, tform, class = lowerDiag) ! whole sample transformation.")
                            affinity = getAffinity(sample, dim, tform, class = lowerDiag) ! whole sample transformation.
            call disp%show("transpose(affinity)")
            call disp%show( transpose(affinity) )
            call disp%show("do isam = 1, nsam")
            call disp%show("    affinity(:,isam) = getAffinity(sample(:,isam), tform, lowerDiag) ! point-wise transformation")
            call disp%show("end do")
                            do isam = 1, nsam
            ...
            call disp%show("transpose(affinity)")
            call disp%show( transpose(affinity) )
            call disp%show("affinInv = getAffinity(affinity, dim, getMatInv(tform), class = lowerDiag) ! inverse transformation.")
            ...
            call disp%show( transpose(sample) )
            call disp%skip()
            call disp%show("affinity = getAffinity(sample, dim, tform, class = lowerUnit) ! whole sample transformation.")
                            affinity = getAffinity(sample, dim, tform, class = lowerUnit) ! whole sample transformation.
            call disp%show("transpose(affinity)")
            call disp%show( transpose(affinity) )
            call disp%show("do isam = 1, nsam")
            call disp%show("    affinity(:,isam) = getAffinity(sample(:,isam), tform, lowerUnit) ! point-wise transformation")
            call disp%show("end do")
                            do isam = 1, nsam
            ...
            call disp%show("transpose(affinity)")
            call disp%show( transpose(affinity) )
            call disp%show("affinInv = getAffinity(affinity, dim, getMatInv(tform), class = lowerUnit) ! inverse transformation.")
        ...
        do ishape = 1, size(shapes)
            ...
            do itform = 1, size(tforms)
                if (tforms(itform)%val == "rotation") then
                    ...
                    affinity = getAffinity(sample, dim, tform, genrecmat, tlate)
                elseif (tforms(itform)%val == "warp") then
                    cov = reshape([1., .5, .5, 1.], shape = [ndim, ndim]) ! getCovRand(1., ndim)
                    tform = getMatChol(cov, lowDia)
                    affinity = getAffinity(sample, dim, tform, lowerDiag, tlate)
                else
                    error stop "Unrecognized tform."
                end if
                affinInv = getAffinity(getShifted(affinity, dim, -tlate), dim, getMatInv(tform))
                if (0 /= getErrTableWrite(SK_"getAffinity."//tforms(itform)%val//SK_"."//shapes(ishape)%val//SK_".sample.txt", sample, trans)) error stop "Failed table-write."
                if (0 /= getErrTableWrite(SK_"getAffinity."//tforms(itform)%val//SK_"."//shapes(ishape)%val//SK_".affinity.txt", affinity, trans)) error stop "Failed table-write."
                if (0 /= getErrTableWrite(SK_"getAffinity."//tforms(itform)%val//SK_"."//shapes(ishape)%val//SK_".affinInv.txt", affinInv, trans)) error stop "Failed table-write."
            end do
        end do
    end block
