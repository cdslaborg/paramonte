program example
    ...
    call disp%show("rank = getRankStandard(string_SK)")
                    rank = getRankStandard(string_SK)
    call disp%show("rank")
    call disp%show( rank )
    call disp%skip()
    ...
    call disp%show("rank = getRankStandard(vector_SK)")
                    rank = getRankStandard(vector_SK)
    call disp%show("rank")
    call disp%show( rank )
    call disp%skip()
    ...
    call disp%show( vector_IK )
    call disp%show("rank = getRankStandard(vector_IK)")
                    rank = getRankStandard(vector_IK)
    call disp%show("rank")
    call disp%show( rank )
    call disp%skip()
    ...
    call disp%show( vector_LK )
    call disp%show("rank = getRankStandard(vector_LK)")
                    rank = getRankStandard(vector_LK)
    call disp%show("rank")
    call disp%show( rank )
    call disp%skip()
    ...
    call disp%show( vector_RK )
    call disp%show("rank = getRankStandard(vector_RK)")
                    rank = getRankStandard(vector_RK)
    call disp%show("rank")
    call disp%show( rank )
    call disp%skip()
    ...
    call disp%show("vector_IK")
    call disp%show( vector_IK )
    call disp%show("rank = getRankStandard(vector_IK, isSorted_IK)")
                    rank = getRankStandard(vector_IK, isSorted_IK)
    call disp%show("rank")
    call disp%show( rank )
    call disp%skip()
    ...
    call disp%show("vector_RK")
    call disp%show( vector_RK )
    call disp%show("rank = getRankStandard(vector_RK, isSorted_RK)")
                    rank = getRankStandard(vector_RK, isSorted_RK)
    call disp%show("rank")
    call disp%show( rank )
    call disp%skip()
    ...
    call disp%show("string_SK")
    call disp%show( string_SK, deliml = SK_"""" )
    call disp%show("rank = getRankStandard(string_SK, isSortedChar1)")
                    rank = getRankStandard(string_SK, isSortedChar1)
    call disp%show("rank")
    call disp%show( rank )
    call disp%skip()
    ...
    call disp%show("string_SK")
    call disp%show( string_SK, deliml = SK_"""" )
    call disp%show("rank = getRankStandard(string_SK, isSortedChar2)")
                    rank = getRankStandard(string_SK, isSortedChar2)
    call disp%show("rank")
    call disp%show( rank )
    call disp%skip()
    ...
    block
        ...
        call disp%show("rank = getRankStandard(cssvec)")
                        rank = getRankStandard(cssvec)
        call disp%show("rank")
        call disp%show( rank )
        call disp%skip()
    end block
