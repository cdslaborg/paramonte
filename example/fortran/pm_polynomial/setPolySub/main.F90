...
block; \
    ...
    call disp%show( getPolyStr(Rhs) ); \
    call disp%show("allocate(Sub(1 : max(size(Lhs), size(Rhs))))"); \
                    allocate(Sub(1 : max(size(Lhs), size(Rhs)))); \
    call disp%show("call setPolySub(Sub, Lhs, Rhs)"); \
                    call setPolySub(Sub, Lhs, Rhs); \
