program example
    ...
    block
        ...
        call disp%show( array )
        call disp%show("maxval(array)")
        call disp%show( maxval(array) )
        call disp%show("maxval(array, dim = 1_IK)")
        call disp%show( maxval(array, dim = 1_IK) )
        call disp%show("maxval(array, dim = 2_IK)")
        call disp%show( maxval(array, dim = 2_IK) )
        call disp%show("maxval([complex(CKG)::])")
        call disp%show( maxval([complex(CKG)::]) )
        call disp%show("maxval([complex(CKG)::], dim = 1_IK)")
        call disp%show( maxval([complex(CKG)::], dim = 1_IK) )
        call disp%show("maxval([complex(CKG)::], dim = 2_IK)")
        call disp%show( maxval([complex(CKG)::], dim = 2_IK) )
        call disp%skip()
    end block
