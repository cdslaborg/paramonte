program example
    ...
    block
        ...
        call disp%show( array )
        call disp%show("minval(array)")
        call disp%show( minval(array) )
        call disp%show("minval(array, dim = 1_IK)")
        call disp%show( minval(array, dim = 1_IK) )
        call disp%show("minval(array, dim = 2_IK)")
        call disp%show( minval(array, dim = 2_IK) )
        call disp%show("minval([complex(CKC)::])")
        call disp%show( minval([complex(CKC)::]) )
        call disp%show("minval([complex(CKC)::], dim = 1_IK)")
        call disp%show( minval([complex(CKC)::], dim = 1_IK) )
        call disp%show("minval([complex(CKC)::], dim = 2_IK)")
        call disp%show( minval([complex(CKC)::], dim = 2_IK) )
        call disp%skip()
    end block
